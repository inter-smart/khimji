    ...
    <section className="w-full h-auto py-[40px] sm:py-[70px_60px] lg:py-[100px_80px] 2xl:py-[120px_100px] 3xl:py-[155px_130px] overflow-hidden block relative z-0">
      ...
      <div className="container">
        <div className="w-full h-auto sm:px-[50px] lg:px-[70px] xl:px-[90px] 2xl:px-[110px] 3xl:px-[140px]">
          <Heading as="h2" size="heading1" className="!text-[#0B436A] !mb-[20px] lg:!mb-[30px] 3xl:!mb-[40px]">
            {cms?.title}
          </Heading>
          <Accordion type="single" collapsible defaultValue={`item-${data?.[0]?.id}`}>
            {data?.map((item) => (
              <AccordionItem
                value={`item-${item.id}`}
                key={item.id}
              ...
              >
                <AccordionTrigger className="[&>svg]:text-[#132C26] 2xl:[&>svg]:w-[17px] 2xl:[&>svg]:h-[17px]">
                  <div className="text-[16px] 2xl:text-[18px] 3xl:text-[22px] leading-[1.4] font-normal text-black max-w-[90%]">{item?.question}</div>
                </AccordionTrigger>
                <AccordionContent>
                  <div
